...
const Admin = () => {
  ...
  const uploadPhoto = async(e) => {
    ...
    console.log('encodeURIComponent(file.name)', filename)
    const res = await fetch(`/api/upload-image?file=${filename}`)
    console.log('fetched presignedPost', res)
    const data = await res.json()
    console.log('presignedPost json()', data)
    const formData = new FormData()
  
    Object.entries({...data.fields, file}).forEach(([key, value]:[key:any, value:any]) => {
      formData.append(key, value)
    })
    console.log('image formData', formData)
    toast.promise(fetch(data.url, {
      method: 'POST',
      body: formData
    }), {
      loading: 'Uploading...',
  ...
  const onSubmit = async(data) => {
    ...
    console.log('form submit image', image)
    ...
    try {
      ...
      console.log('createLink onSubmit error', error)
    }
  }
  return (
          ...
          {loading ? (
            <span className='flex items-center justify-center'>
              <svg
                ...
                <path d="M11 17a1 1 0 001.447.894l4-2A1 1 0 0017 15V9.236a1 1 0 00-1.447-.894l-4 2a1 1 0 00-.553.894V17zM15.211 6.276a1 1 0 000-1.788l-4.764-2.382a1 1 0 00-.894 0L4.789 4.488a1 1 0 000 1.788l4.764 2.382a1 1 0 00.894 0l4.764-2.382zM4.447 8.342A1 1 0 003 9.236V15a1 1 0 00.553.894l4 2A1 1 0 009 17v-5.764a1 1 0 00-.553-.894l-4-2zM11 17a1 1 0 001.447.894l4-2A1 1 0 0017 15V9.236a1 1 0 00-1.447-.894l-4 2a1 1 0 00-.553.894V17zM15.211 6.276a1 1 0 000-1.788l-4.764-2.382a1 1 0 00-.894 0L4.789 4.488a1 1 0 000 1.788l4.764 2.382a1 1 0 00.894 0l4.764-2.382zM4.447 8.342A1 1 0 003 9.236V15a1 1 0 00.553.894l4 2A1 1 0 009 17v-5.764a1 1 0 00-.553-.894l-4-2z"/>
              </svg>
              Creating...
            </span>
          ) : (
...
export const getServerSideProps = async({req, res}) => {
  ...
  console.log('admin.tsx getSession', session)
  if (!session) {
    return {
      redirect: {
        permanent: false,
